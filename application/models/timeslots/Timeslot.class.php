...
*/
	...
	/**
	 * Retourne le nom d'un member
	 * ED 140901
	 * ED 141217 : les membres nesont pas toujours dans le mme ordre et, parfois, c'est le user qui est en tte 
	 */
	function getMemberName($index = NULL){
		$members = $this->getMembers();
		if($index === NULL){
			//cherche le 1er membre du type 1 (contexte)
			foreach($members as $member){
				if($member->getObjectTypeId() == 1){
					$index = -1;
					break;
				}
			}
		...
		return $member->getColumnValue('name');
	}
} // Timeslot
