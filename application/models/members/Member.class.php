...
class Member extends BaseMember {
	...
	/**
	 * @abstract Duplicates member
	 * @param $destination_parent
	 * @param $newName
	 * @return new member
	 * @author ED150217
	 */
	function duplicate($destination_parent, $newName = FALSE){
		$children = $this->getAllChildren();
		$newMember = new Member();
		$newMember->setNew(true);
		
		if($newName)
			$newMember->setName($newName);
		else
			$newMember->setName($this->getName());
		$newMember->setDepth($this->getDepth());
		$newMember->setColor($this->getColor());	
		$newMember->setDimensionId($this->getDimensionId());
		if(!$destination_parent)
			$newMember->setParentMemberId(1);
		else
			$newMember->setParentMemberId($destination_parent->getId());
			
		$newMember->setObjectTypeId($this->getObjectTypeId());
		
		if($itemClass = $this->getObjectClass()) {
			eval ("\$newObjectMember = new $itemClass();");
			//$newObjectMember = new ObjectMember();
			$newObjectMember->setNew(true);
			//$newObjectMember->setObjectTypeId($this->getObjectTypeId());
			$newObjectMember->setObjectName($newMember->getName());
			$newObjectMember->save();
		
			$newMember->setObjectId($newObjectMember->getObjectId());
		}
		
		$newMember->save();
		if(!$newMember->getId())
			var_dump($newMember->getName() . ' n\'a pas d\'id !!!!');
		//Duplicate children
		foreach($children as $child){
			$child->duplicate($newMember);
		}
		
		$this->duplicatePropertyValues($newMember);
		$this->duplicateContactsPermissions($newMember);
		
		return $newMember;
	}

	/**
	 * @abstract Duplicates member property values
	 * @param $destination_member
	 * @return nil
	 * @author ED150217
	 */
	function duplicatePropertyValues($destination_member){
		
		$sql = "INSERT INTO fo_member_custom_property_values (`member_id`, `custom_property_id`, `value`)
			SELECT ?, `custom_property_id`, `value`
			...
			";
		return DB::execute($sql, array($destination_member->getId(), $this->getId(), $destination_member->getId()));
	}

	/**
	 * @abstract Duplicates contacts member permissions
	 * @param $destination_member
	 * @return nil
	 * @author ED150217
	 */
	function duplicateContactsPermissions($destination_member){
		
		$sql = "INSERT INTO `fo_contact_member_permissions` (`permission_group_id`, `member_id`, `object_type_id`, `can_write`, `can_delete`)
			SELECT `permission_group_id`, ?, `object_type_id`, `can_write`, `can_delete`
			FROM fo_contact_member_permissions
			WHERE member_id = ?
			AND NOT EXISTS (
				SELECT 1
				FROM fo_contact_member_permissions
