	...
	<?php endif;?>
			...
			<div id="<?php echo $genid; ?>add-member-form-dim<?php echo $dimension_id?>" class="" style="display:<?php echo ($form_visible?'block':'none')?>;">
				...
				$combo_listeners = array(
					...
					"blur" => "function (combo) { var rec = combo.store.getAt(0); if (combo.getValue().trim() != '' && rec) {
						combo.select(0, true); combo.fireEvent('select', combo, rec, 0); } }"
				);
				...
				* */
				...
				echo autocomplete_member_combo("member_autocomplete-dim".$dimension_id, $dimension_id, $autocomplete_options,
					$empty_text, array('class' => 'member-name-input'
							   ...
							   , 'tabIndex' => 1000 ,'style' => 'width : 240px','is_ajax' => true)
					, true, $genid .'add-member-input-dim'. $dimension_id, $combo_listeners);?>
				<div class="clear"></div>
			</div>
		</div>
	</div>
