...
class DimensionController extends ApplicationController {
	...
	*/
	...
	function initial_list_dimension_members_tree() {
		...
		/* ED141209 */
		$current_module = array_var($_REQUEST, 'current_module', false);
		$isTimeSlot = $current_module == 'timeslot';
		...
		//ED141120, ED141204 dans le cas des timeslots : suppression des ŽlŽments '(ne pas utiliser)'
		// et obligation de saisir dans '3. Suivi du temps de travail'
		if($isTimeSlot)
		foreach($memberList as $key=>$member)
			if((strpos($member["to_show"], '3. Suivi du temps de travail') === FALSE
			|| strpos($member["to_show"], '(ne pas utiliser)')))
