...
class FilesController extends ApplicationController {
	...
	function add_multiple_files() {
		...
		if (is_array(array_var($_POST, 'file'))) {
				...
				echo '$uploaded_file '; var_dump($uploaded_file);
				echo '$_FILES '; var_dump($_FILES);
				echo '$_SESSION '; var_dump($_SESSION);
				echo '</pre>';
				
				die('Desole, cette fonctionnalite est hors service depuis la derniere mise a jour.');*/
		...
		if (!empty($uploaded_file['tmp_name'])) {
			if($multiple){
				foreach ($fname as $key => $file) {
					//echo '<br>copy multiple ' . $uploaded_file['tmp_name'][$key] . ' --> ' . $file ;
					copy($uploaded_file['tmp_name'][$key], $file);
				}
			}else{
				//echo '<br>copy ' . $uploaded_file['tmp_name'] . ' --> ' . $fname ;
			...
			//echo '<br>$id = ' . $id . '<br>' . print_r($_SESSION[$id], true);
		}
	}
