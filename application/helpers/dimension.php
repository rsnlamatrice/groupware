...
function render_member_selectors($content_object_type_id, $genid = null, $selected_member_ids = null, $options = array(), $skipped_dimensions = null, $simulate_required = null, $default_view = true) {
	if (is_numeric($content_object_type_id)) {
		...
		/*
		echo 'active_context() = <pre>';
		var_dump(active_context());
		echo '</pre><br>';
		*/
		...
		if ($dimensions != null && count($dimensions)) {
			...
			/* ED141209 add $current_module variable
			 * valeur dŽfinie dans time/index.php
			 * utilisŽe dans le template multiple_dimension_selector ...> small_view */
			$current_module = array_var($options, 'current_module', array());
